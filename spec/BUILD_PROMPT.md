...
## Routes / screens
> Confirmed live route slugs (see SCREEN_NOTES.md for full per-screen detail): Shop `/shop`,
> Winemaker `/winemaker-spotlight`, SESH `/current-offer/[id]`, Sign up `/register_details`,
> Cart `/customer-cart`, Admin `/admin`, plus a 404 page and a `/site-access` password gate.
